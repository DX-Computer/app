...
const CreateAgentLeft: FunctionComponent = (): JSX.Element => {
  const s = useShell();
  ...
  return (
    <>
      ...
      >
        ...
        >
          {s.dict.createAgent.navAgents}
        </Link>
      </Caja>

      <Caja className="flex-col flex-1 md:min-h-0 md:overflow-y-auto gap-3 p-3">
        <span className="relative flex text-sm">{s.dict.createAgent.howAgentsWork}</span>
        <div className="relative flex flex-col gap-1">
          <span className={tag}>{s.dict.createAgent.openSource}</span>
          <span className="relative flex text-xs leading-relaxed">
            {s.dict.createAgent.openSourceBody}
          </span>
        </div>
        <div className="relative flex flex-col gap-1">
          <span className={tag}>{s.dict.createAgent.rewards}</span>
          <span className="relative flex text-xs leading-relaxed">
            {s.dict.createAgent.rewardsBody}
          </span>
        </div>
      </Caja>
