...
import { TreelinerStats } from "../types/common.types";
...
const TreelinerLeft: FunctionComponent<{ stats: TreelinerStats }> = ({
  stats,
}): JSX.Element => {
  const s = useShell();
  ...
  return (
    <>
      ...
      >
        ...
        >
          {s.dict.treeliner.treelinerGrants}
        </Link>
      </Caja>

      <Caja className="flex-col flex-1 md:min-h-0 md:overflow-y-auto gap-3 p-3">
        <span className="relative flex text-sm">{s.dict.treeliner.treeliner}</span>
        <div className="relative flex flex-col gap-1">
          <span className={tag}>{s.dict.treeliner.address}</span>
          {href ? (
            <a
              href={href}
              target="_blank"
        ...
        <div className="relative flex flex-col gap-2">
          <div className="relative flex flex-row items-center gap-2">
            <span className={`${tag} flex-1`}>{s.dict.treeliner.grantsFunded}</span>
            <span className="relative flex text-xs">{stats.grantsFunded}</span>
          </div>
          <div className="relative flex flex-row items-center gap-2">
            <span className={`${tag} flex-1`}>{s.dict.treeliner.staked}</span>
            <span className="relative flex text-xs">
              {fmt(s.dict.treeliner.monaValue, { value: stats.totalStaked })}
            </span>
          </div>
          <div className="relative flex flex-row items-center gap-2">
            <span className={`${tag} flex-1`}>{s.dict.treeliner.claimed}</span>
            <span className="relative flex text-xs">
              {fmt(s.dict.treeliner.monaValue, { value: stats.totalClaimed })}
            </span>
          </div>
        </div>
      </Caja>
